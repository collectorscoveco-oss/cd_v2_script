...
The update button also handles installed users cleanly: dev checkouts keep using `git pull --ff-only` plus `npm install --prefix ui`, while installed users open the latest release page so they can download the refreshed Windows installer and rerun it.
