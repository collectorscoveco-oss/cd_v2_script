...
The update button also handles release ZIPs cleanly: dev checkouts keep using `git pull --ff-only` plus `npm install --prefix ui`, while release ZIPs open the latest release page so the user can download the refreshed ZIP and rerun the release launcher.
