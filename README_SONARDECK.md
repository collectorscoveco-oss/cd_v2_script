...
The native desktop direction is Option 1: Tauri + React/TypeScript, with Option 2 (C#/.NET WPF or WinUI) saved for later if Tauri does not work out. Start native desktop mode with `scripts\run_desktop_app.bat`; see `docs\native-desktop.md`. GitHub desktop package/release automation lives in `.github\workflows\sonardeck-desktop-package.yml`, and contributors are tracked in `CONTRIBUTORS.md`.

The modern UI prototype starts a Python local API plus a Vite/React frontend. It currently supports profile switching, polished virtual deck cards, firing mapped buttons, a simplified Button Setup panel for remapping, editable App/Website paths and protocols, manual `.exe` app action creation by pasting a full Windows path, keyboard-shortcut button creation/editing, direct launching for pasted `.exe` paths, brand/logo icons for common apps and future action fallbacks, automatic icon matching from app paths, manual per-button icon override persistence, a Check / Install Updates button, right-click display-name editing, and a wide Button 10 card where short click is Play/Pause and hold is Next Page. Use the UI's bridge/server URL override when the deck UI and bridge live on different PCs, when the bridge is on a separate server PC from the gaming PC, or when the bridge is reached through a Cloudflare tunnel. On installed releases, the bridge/server URL is usually the LAN address on port 8766, not `127.0.0.1` from another device. See `docs\modern-ui.md`, `docs\icons.md`, and `docs\ten-button-layout.md`.

The update button also handles release ZIPs cleanly: dev checkouts keep using `git pull --ff-only` plus `npm install --prefix ui`, while release ZIPs open the latest release page so the user can download the refreshed ZIP and rerun `scripts\run_release.bat`.
