...
The virtual controller now includes live Sonar mixer bars, profile-colored pages, polished Stream Deck-style button cards with no hover color changes, a top toolbar with Check / Install Updates, an Edit Mapping Mode where clicking a deck button selects it for editing, editable button names and mappings, an Action Type filter, GUI builders for custom app/website/hotkey buttons, profile/page create/duplicate/rename tools, config backup/restore, F1-F9/Ctrl+Alt shortcuts, and profile-switch audio modes (`off`, `beep`, `profile_beeps`, `terminal_bell`, `system`, `file`, `profile_files`, `voice`).
