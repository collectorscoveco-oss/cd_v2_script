...
The update button also handles release installers cleanly: dev checkouts keep using `git pull --ff-only` plus `npm install --prefix ui`, while installed releases open the latest release page so the user can download the refreshed installer and rerun it.
