...
The modern UI prototype starts a Python local API plus a Vite/React frontend. It currently supports profile switching, polished virtual deck cards, firing mapped buttons, a simplified Button Setup panel for remapping, editable App/Website paths and protocols, manual `.exe` app action creation by pasting a full Windows path, direct launching for pasted `.exe` paths, brand/logo icons for common apps and future action fallbacks, automatic icon matching from app paths, manual per-button icon picker overrides, per-button color overrides, a Check / Install Updates button, right-click display-name editing, and a wide Button 10 card where short click is Play/Pause and hold is Next Page. See `docs\modern-ui.md`, `docs\icons.md`, and `docs\ten-button-layout.md`.
