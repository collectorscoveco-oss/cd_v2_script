...
- Sidebar Check / Install Updates button runs `git pull --ff-only` and `npm install --prefix ui` through the local bridge API when this is a git checkout; installed releases open the latest GitHub release page so end users can download the Windows installer and rerun it
