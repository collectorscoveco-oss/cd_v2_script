...
- Sidebar Check / Install Updates button runs `git pull --ff-only` and `npm install --prefix ui` through the local bridge API when this is a git checkout; release ZIPs open the latest GitHub release page so end users can download the refreshed ZIP and rerun `scripts/run_release.bat`
